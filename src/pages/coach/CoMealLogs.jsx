...
function CoMealLogs() {
  ...
  const fetchPlansWithDetails = async () => {
    try {
      const res = await api.get("/nutrition/mealplans");
      const userPlans = res.data.plans || [];
      const plansWithDetails = await Promise.all(
        userPlans.map(async (plan) => {
          //const planRes = await api.get(`/nutrition/${plan.plan_id}`);
          //return planRes.data;
        })
      );
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    try {
      await api.post("/nutrition/mealplans", newPlan);
      setPopOpen(null);
  ...
  const handleSelectPlan = async (plan_id) => {
    try {
      const res = await api.get(`/nutrition/mealplans/${plan_id}`);
      const plan = res.data;
  ...
  const handleUpdate = async () => {
    try {
      await api.patch(`/nutrition/mealplans/${selectedPlan.plan_id}`, editData);
      await fetchPlansWithDetails();
  ...
  const handleDeletePlan = async (plan_id) => {
    try {
      await api.delete(`/nutrition/plans/${plan_id}`);
      await fetchPlansWithDetails();
  ...
  const handleAddDay = async (plan_id) => {
    ...
    try {
      await api.post(`/nutrition/plans/${plan_id}/days`, {
  ...
  const handleDeleteDay = async (plan_id, plan_day_id) => {
    try {
      await api.delete(`/nutrition/plans/${plan_id}/days/${plan_day_id}`);
      await fetchPlansWithDetails();
      await handleSelectPlan(plan_id);
  ...
  const handleUpdateDayExercise = async () => {
    ...
    try {
      await api.patch(
        `/nutrition/plans/${plan_id}/days/${day_id}/exercises/${de_id}`,
        {
  ...
  const handleDeleteDayExercise = async (plan_id, day_id, de_id) => {
    try {
      await api.delete(`/nutrition/plans/${plan_id}/days/${day_id}/exercises/${de_id}`);
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      ...
      <div className="flex justify-between items-center">
        <div className="text-2xl font-bold">Meal Plans</div>
        {selectedClient && (
