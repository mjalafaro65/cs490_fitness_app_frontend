...
function CProfile() {
  ...
  const handlePayInvoice = async (invoiceId) => {
    ...
    try {
      const res = await api.post("/client/pay-invoice", {
        invoice_id: invoiceId,
      });

      showAlert(res.data.message || "Payment successful", "success");

      // instant UI update (better UX than refetch first)
      setInvoices(prev =>
        prev.map(inv =>
  ...
  const hasPayIssue = invoices.some(
    i =>
      i.status?.toLowerCase() === "issued" ||
      i.status?.toLowerCase() === "past_due"
  );
      ...
      <div className="drawer-content">
        <section className="p-6 flex flex-col gap-6">
          ...
          <div className="flex gap-6">
            <button
              className="btn btn-primary text-white bg-blue-800 btn-m rounded-t relative"
            ...
            <button className="btn btn-primary text-white bg-blue-800 btn-m rounded-t" onClick={handleOpenPayments}>Payments</button>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium">
                        Coach: {invoice.coach_name || "Unknown"}
                      </p>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {invoice.status?.toLowerCase() === "issued" && (
                        <button
                          ...
                          className="btn btn-sm bg-blue-800 text-white hover:bg-blue-900 disabled:opacity-50"
                        >
                ...
                {payments.map((payment, index) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {/* STATUS BADGE */}
                      <span
                        className={`badge ${payment.status === "disputed"
                          ? "badge-warning"
                      ...
                      {payment.can_dispute &&
                        ...
                        payment.status !== "refunded" && (
                          <button
                            ...
                            onClick={() =>
                              setPopOpen(`dispute-${payment.payment_id}`)
                            }
                          >
                            Dispute
                          </button>
