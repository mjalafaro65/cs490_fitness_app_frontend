...
function ADashboard() {
  ...
  const [workCount, setWork] = useState(0);
  ...
  useEffect(() => {
    const fetchAppli = async () => {
      try {
        setLoading(true);
        const [appli, work] = await Promise.all([
          api.get("/admin/coach-applications"),
          api.get("/workouts/exercises")
        ]);

        console.log("number", work);
        
        const pending = appli.data.filter((app) => app.status === "pending");
        const workoutCount = work.data.exercises?.length;
        setAppli(pending);
        setWork(workoutCount);
      } catch (err) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              <div className="card bg-base-200 shadow-lg border border-base-500 rounded-box p-5">
                ...
                {loading ? (
                  <p className="text-sm opacity-50">Loading applications...</p>
                ) : appli?.length > 0 ? (
                  <p className="text-3xl font-bold text-blue-800">{appli.length}</p>
                ) : (
                  <p className="text-sm opacity-50">0 applications</p>
                )}

                <p className="text-xs opacity-50 mt-1">
                  Pending applications to moderate
                </p>

                <button
                  className="mt-4 w-full btn bg-gray-800 text-white btn-sm hover:bg-bleu-700"
              ...
              <div className="card bg-base-200 shadow-lg border border-base-500 rounded-box p-5">
                ...
                {loading ? (
                  ...
                  <p className="text-3xl font-bold text-blue-800">{workCount}</p>
                ) : (
                  <p className="text-sm opacity-50">0 workouts</p>
                )}
                ...
                <div className="flex gap-2 mt-4">
                  <button
                    className="flex-1 btn bg-gray-800 text-white btn-sm"
            ...
            <>
              {stats?.total_users > 0 && (
                <div className="mt-8 card bg-base-200 shadow-lg border border-base-500 rounded-box p-6">
                  ...
                  <div className="mt-6 pt-4 border-t border-base-800 grid grid-cols-3 gap-2">
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold text-gray-600">
                        +{stats.new_client_users_last_7 || 0}
                      </p>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs opacity-60 mt-1">Deleted this week</p>
                    </div>
                  </div>
                </div>
              )}
